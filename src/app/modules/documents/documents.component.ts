...
export class DocumentsComponent implements OnInit {
  ...
  // ---------------- ✅ INPUT SANITIZERS ----------------
  onLettersInput(controlName: string): void {
    const ctrl = this.requestForm.get(controlName);
    if (!ctrl) return;
  ...
  onDigitsInput(controlName: string, maxLen: number): void {
    const ctrl = this.requestForm.get(controlName);
    if (!ctrl) return;
  ...
  // ---------------- REVIEW FLOW ----------------
  startReview(): void {
    // ✅ sanitize key text fields before validating
    this.onLettersInput('firstName');
    this.onLettersInput('lastName');
    this.onLettersInput('childName');
    this.onLettersInput('groom');
    this.onLettersInput('bride');
    this.onLettersInput('deathName');
    this.onLettersInput('relation');

    if (this.isGcash) {
      this.onDigitsInput('gcashReferenceNumber', 13);
    }
  ...
  private persistRequest(): void {
    // ✅ sanitize again to avoid invalid paste before submit
    this.onLettersInput('firstName');
    this.onLettersInput('lastName');
    if (this.isGcash) this.onDigitsInput('gcashReferenceNumber', 13);
  ...
  updateValidators(): void {
    ...
    // ✅ STRICT: names letters-only
    this.requestForm.get('firstName')?.setValidators([Validators.required, Validators.pattern(this.namePattern)]);
    this.requestForm.get('lastName')?.setValidators([Validators.required, Validators.pattern(this.namePattern)]);

    this.requestForm.get('paymentMethod')?.setValidators([Validators.required]);

    // ✅ STRICT: gcash = exactly 13 digits
    if (this.isGcash) {
      this.requestForm.get('gcashReferenceNumber')?.setValidators([
        Validators.required,
        Validators.pattern(this.gcash13Pattern)
      ]);
    }

    if (this.selectedType === 'baptismal' || this.selectedType === 'confirmation') {
      this.requestForm.get('childName')?.setValidators([Validators.required, Validators.pattern(this.namePattern)]);
    ...
    if (this.selectedType === 'wedding') {
      this.requestForm.get('groom')?.setValidators([Validators.required, Validators.pattern(this.namePattern)]);
      this.requestForm.get('bride')?.setValidators([Validators.required, Validators.pattern(this.namePattern)]);
    ...
    if (this.selectedType === 'death') {
      this.requestForm.get('deathName')?.setValidators([Validators.required, Validators.pattern(this.namePattern)]);
      ...
      // relation can be words like "Father", "Aunt", etc.
      this.requestForm.get('relation')?.setValidators([Validators.required, Validators.pattern(this.namePattern)]);
